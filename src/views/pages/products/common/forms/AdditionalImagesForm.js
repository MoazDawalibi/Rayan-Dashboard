...
const dataUrl = "add_products_additional_images";
...
const AdditionalImagesForm = ({ images, setImages }) => {
  ...
  const onChange = (imageList, addUpdateIndex) => {
    ...
    if (imageList.length === 0) {
      formik.setFieldValue("product_additional_images", []);
      return;
    }
    ...
    const formImages = formik.values.product_additional_images;
    //Add Operation
    if (imageList.length > formImages.length) {
      ...
      formik.setFieldValue("product_additional_images", newImages);
    }
    //Update Operation
  ...
  const handleDelete = (index) => {
    formik.setFieldValue(
      ...
      formik.values.product_additional_images.filter((i, idx) => index !== idx)
    );
  };
